...
def test_dtproj_parsing_incorrectly_linked(tmpdir, dtproj_file_1):
    ...
    ssis_project = ssis_validator.Deployment._parse_dtproj_file(dtproj)
    ...
    assert ssis_project.location == dtproj_path
...
def test_dtsx_parsing(tmpdir, dtsx_file_1):
    ...
    dtsx = ssis_validator.Deployment._parse_dtsx_file(ssis_package)

    assert dtsx.name == dtsx_path.stem
    assert dtsx.location == dtsx_path
